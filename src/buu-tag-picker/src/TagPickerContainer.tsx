...
export default class TagPickerContainer extends Component<TagPickerProps, TagPickerState> {
    controlRef = React.createRef<HTMLButtonElement>()
    ...
    handleSearchChange = (value: string) => this.setState({
        query: value,
        hovered: 0,
        filteredData: this.props.data.filter(tag => tag.name.toLowerCase().trim().includes(value.toLowerCase().trim())),
        canCreate: value.trim().length > 0 && this.props.data.every(
    ...
    render(): React.ReactNode {
        ...
        const filteredData = this.props.data.filter(tag => tag.name.toLowerCase().trim().includes(query.toLowerCase().trim()))
